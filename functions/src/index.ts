...
export const processUploadedImage = functions.storage.object().onFinalize(async (object) => {
  ...
  if (object.name.startsWith("processed/")) {
    console.log("File is already in processed folder, skipping:", object.name);
    return null;
  }
  ...
  try {
    ...
    await bucket.file(filePath).move(`processed/${userId}/${path.basename(filePath)}`);
...
export const findUserByEmail = functions.https.onCall(async (data, context) => {
  // Hitelesítés ellenőrzése
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Hitelesítés szükséges");
  }

  const email = data.email?.toLowerCase().trim();
  if (!email) {
    throw new functions.https.HttpsError("invalid-argument", "Érvénytelen email cím");
  }
  try {
    ...
    throw new functions.https.HttpsError("not-found", "Nem található felhasználó");
  }
});
...
export const sendPushNotification = functions.https.onCall(async (data, context) => {
  ...
  if (!expoPushToken) {
    ...
    throw new functions.https.HttpsError("not-found", "Expo push token not found");
  }
  const message = {
    to: expoPushToken,
    sound: "default",
